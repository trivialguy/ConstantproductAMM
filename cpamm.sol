...
contract cpamm{
    ...
    function swap(address _tokenIn, uint _amountIn) external returns (uint amountOut) {
        require(_tokenIn==address(token0) || _tokenIn==address(token1),"Invalidate token");
    ...
    function addLiquidity(uint _amount0, uint _amount1) external returns (uint shares){
        ...
        if(reserve0>0 || reserve1>0) {
            require(reserve0*_amount1==reserve1*_amount0, "might change the value of tokens");
        }
    ...
    function removeLiquidity(uint _shares) external returns (uint amount0, uint amount1){
        ...
        require(amount0>0 && amount1>0, "not enough reserves");
